...
通过 MootdxProvider / TencentFinanceProvider 验证可用的符号。两者都接受 `600519.SH`、`sh600519`、`600519` 这类输入；腾讯请求会自动转换成 `sh600519` / `sz300750`，mootdx 请求会自动剥离交易所前后缀。

| 符号 | 标的 | 适用 provider |
|------|------|---------------|
| `600519.SH` / `sh600519` | 贵州茅台 | MootdxProvider, TencentFinanceProvider |
| `300750.SZ` / `sz300750` | 宁德时代 | MootdxProvider, TencentFinanceProvider |
| `002594.SZ` / `sz002594` | 比亚迪 | MootdxProvider, TencentFinanceProvider |
| `600036.SH` / `sh600036` | 招商银行 | MootdxProvider, TencentFinanceProvider |
| `601318.SH` / `sh601318` | 中国平安 | MootdxProvider, TencentFinanceProvider |
| `000001.SZ` / `sz000001` | 平安银行 | MootdxProvider, TencentFinanceProvider |
| `sh000001` | 上证指数 | MootdxProvider (`is_index=True`), TencentFinanceProvider |
| `sz399001` | 深证成指 | MootdxProvider (`is_index=True`), TencentFinanceProvider |
| `sz399006` | 创业板指 | MootdxProvider (`is_index=True`), TencentFinanceProvider |
| `sh000300` | 沪深300 | MootdxProvider (`is_index=True`), TencentFinanceProvider |

**A 股分析常用组合：**
- `MootdxProvider.get_bars()`：日 K、1m/5m/15m/30m/1h 分钟 K
- `MootdxProvider.get_realtime_quotes()`：通达信实时行情
- `MootdxProvider.get_financial_summary()`：EPS、营收、净利润、总资产、净资产、股本
- `MootdxProvider.get_company_profile()`：F10 公司概况和财报摘要文本
- `TencentFinanceProvider.get_stock_metrics()`：PE、PB、流通/总市值、换手率
