...
For China A-share work, use structured market/fundamental data only: realtime quotes, K-lines, intraday points, transactions, order book, valuation, market cap, turnover, financial summary, share capital/shareholders, and sector quote data. Do not use news, research reports, headlines, or narrative feeds as evidence.
...
- MootdxProvider: A-share daily/weekly/monthly/minute K lines, realtime quotes, intraday points, transaction ticks, five-level order book, index K-lines, local TDX files, EPS/financial summary, shareholder/share-capital snapshot, F10 company profile
- TencentFinanceProvider: A-share realtime snapshots, PE, PB, float/total market cap, turnover rate, volume ratio, index/HK/US quote snapshots, explicit board raw data endpoints
- YahooPriceProvider: FXI/KWEB/ASHR or offshore China ETFs for foreign capital risk pricing
- Currencies: USDCNY=X for RMB pressure/capital-flow proxy
- Treasury/BIS/World Bank: macro backdrop for rates, credit cycle, GDP trend
- Web search: only for sector-specific price/market data when no structured provider exists; do not cite news/articles/analyst opinions as evidence
...
from digital_oracle import (
    ...
    MootdxTransactionQuery, MootdxCompanyProfileQuery,    # requires uv pip install mootdx
...
mootdx = MootdxProvider()  # requires uv pip install mootdx
...
result = gather({
    ...
    # China A-share data
    "a_share_daily": lambda: mootdx.get_bars(MootdxBarQuery(symbol="600519.SH", frequency="day", offset=120)),
    "a_share_intraday": lambda: mootdx.get_intraday_points(MootdxIntradayQuery(symbol="600519.SH")),
    "a_share_ticks": lambda: mootdx.get_transactions(MootdxTransactionQuery(symbol="600519.SH", offset=100)),
    "a_share_order_book": lambda: mootdx.get_order_books("600519.SH"),
    "a_share_valuation": lambda: tencent.get_stock_metrics(TencentStockMetricsQuery(symbols=("600519.SH", "300750.SZ"))),
    "a_share_fundamentals": lambda: mootdx.get_financial_summary("600519.SH"),
    "a_share_shareholders": lambda: mootdx.get_shareholder_snapshot("600519.SH"),
    "a_share_profile": lambda: mootdx.get_company_profile(MootdxCompanyProfileQuery(symbol="600519.SH", sections=("公司概况", "最新提示"))),
...
| **MootdxProvider** | **China A-share data** | **K-lines, intraday, ticks, order book, EPS, shareholders, F10** | **mootdx** |
| **TencentFinanceProvider** | **China A-share valuation** | **Realtime quote, PE, PB, market cap, turnover, volume ratio** | **stdlib** |

> 13 out of 16 providers have zero external dependencies and zero API keys. YahooPriceProvider and YFinanceProvider require `pip install yfinance`; MootdxProvider requires `pip install mootdx`.
...
- MootdxProvider requires `uv pip install --target .deps mootdx`. It accepts A-share symbols like `600519.SH`, `sh600519`, or `600519`; use `MootdxBarQuery(..., is_index=True)` for indices such as `sh000001`
- MootdxProvider can also read local TDX `.day` and minute files through `read_local_bars(MootdxLocalDataQuery(...))`; local paths are only used when explicitly supplied by the user
- TencentFinanceProvider has no external dependency. It returns PE/PB/market cap/turnover/volume ratio from Tencent's realtime quote endpoint; market cap fields are in 100 million CNY and amount is in 10 thousand CNY
- For A-share work, do not fetch news pages, headlines, research reports, or narrative feeds. Use only market behavior and structured company/financial data.
