...
Digital Oracle turns this insight into an executable tool. It plugs into 16 authoritative financial data sources — **from prediction markets like Polymarket and Kalshi, to US Treasury yield curves, CFTC institutional positioning, SEC insider trades, central bank rates, crypto derivatives, and China A-share market/fundamental data.**
...
| MooTDX | China A-share data | Daily/minute K, realtime quotes, EPS, F10 profile |
| Tencent Finance | China A-share valuation | PE, PB, market cap, turnover rate |
...
- 13 out of 16 data sources have zero external dependencies (pure Python stdlib). Options chain analysis and MooTDX A-share data require extra installs:

```bash
uv pip install yfinance
uv pip install mootdx
```
...
- **Zero dependencies first** — 13/16 providers use only the Python standard library, no `pip install` needed
