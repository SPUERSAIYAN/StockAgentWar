...
mootdx = MootdxProvider()  # requires uv pip install --target .deps mootdx

# 日 K / 分钟 K
bars = mootdx.get_bars(MootdxBarQuery(
    symbol="600519.SH",   # 也接受 sh600519 / 600519
    frequency="day",      # day/week/month/1m/5m/15m/30m/1h
    offset=120,           # 通达信单次最多 800
    adjust="qfq",         # 可选: qfq/hfq
))
# 返回 PriceHistory；bars.bars -> tuple[PriceBar, ...]

minute = mootdx.get_bars(MootdxBarQuery(symbol="300750.SZ", frequency="1m", offset=240))

# 日期区间日 K
ohlc = mootdx.get_ohlc_range(MootdxDateRangeQuery(
    symbol="600519.SH",
    begin="2026-01-01",
    end="2026-05-11",
    adjust="qfq",
...
# 实时行情
quotes = mootdx.get_realtime_quotes(["600519.SH", "300750.SZ"])
# quote.symbol, quote.name, quote.price, quote.previous_close, quote.volume, quote.amount

# 五档盘口
books = mootdx.get_order_books("600519.SH")
# book.levels[0].bid_price, bid_volume, ask_price, ask_volume

# 当日/历史分时
today_points = mootdx.get_intraday_points("600519.SH")
history_points = mootdx.get_intraday_points(MootdxIntradayQuery(symbol="600519.SH", date="20260508"))
# point.time, point.price, point.average_price, point.volume

# 当日/历史分笔
ticks = mootdx.get_transactions(MootdxTransactionQuery(symbol="600519.SH", offset=100))
old_ticks = mootdx.get_transactions(MootdxTransactionQuery(symbol="600519.SH", date="20260508", offset=100))
# tick.time, tick.price, tick.volume, tick.direction

# 基础财务摘要（EPS 会优先读取显式字段；无显式 EPS 时用净利润/总股本估算）
summary = mootdx.get_financial_summary("600519.SH")
# summary.eps, summary.revenue, summary.net_profit, summary.total_assets
# summary.net_assets, summary.total_share_capital, summary.book_value_per_share

# 股东/股本快照、除权除息、股票列表
shareholders = mootdx.get_shareholder_snapshot("600519.SH")
actions = mootdx.get_corporate_actions("600519.SH")
sh_stocks = mootdx.list_stocks("sh")

# 本地通达信文件：读取 vipdoc 下 .day / .1 / .5 / .lc1 / .lc5
local_daily = mootdx.read_local_bars(MootdxLocalDataQuery(
    symbol="600519.SH",
    tdxdir="C:/new_tdx",
    frequency="day",  # day / 1m / 5m
))

# F10 公司概况/财报摘要
profile = mootdx.get_company_profile(MootdxCompanyProfileQuery(
    symbol="600519.SH",
    sections=("公司概况", "最新提示", "财务分析"),
))
# profile.sections, profile.overview
```

**频率映射：**
- `day`/`d` -> 日 K；`week`/`w` -> 周 K；`month`/`m` -> 月 K
- `1m`, `5m`, `15m`, `30m`, `1h` -> 分钟/小时 K
- 指数 K 线用 `MootdxBarQuery(symbol="sh000001", frequency="day", is_index=True)`
...
metrics = tencent.get_stock_metrics(TencentStockMetricsQuery(
    symbols=("600519.SH", "300750.SZ"),
))
# 返回 tuple[TencentStockMetrics, ...]
# m.price, m.change_pct, m.turnover_rate, m.pe, m.pb
# m.float_market_cap_cny_100m, m.total_market_cap_cny_100m, m.volume_ratio

# 指数 / 港股 / 美股实时快照也可用同一实时行情接口
...
**字段单位：**
- `turnover_rate`, `change_pct`, `amplitude_pct`：百分比数值
- `volume_ratio`：量比
- `amount_cny_10k`：万元人民币
