...
Digital Oracle 把这个洞察变成了一个可执行的工具。它接入了 16 个权威金融数据源 — **从 Polymarket 和 Kalshi 这样的预测市场，到美国国债收益率曲线、CFTC 机构持仓、SEC 内部人交易、各国央行利率、加密衍生品，以及中国 A 股行情和基本面数据。**
...
| MooTDX | 中国 A 股数据 | 日 K、分钟 K、实时行情、EPS、F10 公司概况 |
| 腾讯财经 | 中国 A 股估值 | PE、PB、市值、换手率 |
...
- 16 个数据源中有 13 个零外部依赖（纯 Python 标准库）。期权链分析和 MooTDX A 股数据需要额外安装：
...
- **零依赖优先** — 13/16 个 provider 只用 Python 标准库，无需 `pip install`
